...
function Login(props) {
  ...
  //********** appId est un ID qu'on récupère depuis un compte développeur facebook */
  //       appId: "",
  //     });
  ...
  //         `https://graph.facebook.com/${response.id}?fields=id,first_name,last_name,email&access_token=${token}`
  //       );
  //       const userInfo = await rawuserInfo.json();
  //       // AsyncStorage.setItem('user', JSON.stringify(userInfo));
  //       console.log('userInfo',userInfo)
  //       if(userInfo){
  //         var rawFacebookSignin = await fetch(`http://${ip}:3000/users/facebookSignin`,{
  //           method:'post',
  //           headers: { "Content-Type": "application/x-www-form-urlencoded" },
  //           body: `email=${userInfo.email}`
  //         })
  //         var facebookSigninUser = await rawFacebookSignin.json();
  //         console.log('facebookSigninUser:',facebookSigninUser)
  //         if(facebookSigninUser.result === true){
  //           props.addToken(facebookSigninUser.user.token);
  //           props.saveUserInfo({
  //             firstname: facebookSigninUser.user.firstname,
  //             lastname: facebookSigninUser.user.lastname,
  //             ticketsRemaining: facebookSigninUser.user.NumberOfPlacesHave,
  //             renewalDate: facebookSigninUser.user.subscription.dateOfSubscription,
  //             subscriptionName: facebookSigninUser.user.subscription.idSubscription.nom,
  //             email: facebookSigninUser.user.email,
  //           });
  //           if (!props.returnToScreen) {
  //             props.navigation.reset({
  //               index: 0,
